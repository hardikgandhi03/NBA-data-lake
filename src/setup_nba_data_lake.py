...
base_name = os.getenv("AWS_BUCKET_NAME")  
random_suffix = str(uuid.uuid4())[:8]
bucket_name = f"{base_name}-{random_suffix}".lower()
...
def create_s3_bucket():
    ...
    try:
        if not bucket_name:
            raise ValueError("Bucket name is empty. Ensure AWS_BUCKET_NAME is set correctly in the .env file.")
        ...
        # Save the bucket name to the resources.json file
        save_bucket_name(bucket_name)

    except s3_client.exceptions.BucketAlreadyExists as e:
        print(f"Bucket '{bucket_name}' already exists. Error: {e}")
    except s3_client.exceptions.BucketAlreadyOwnedByYou as e:
        print(f"Bucket '{bucket_name}' is already owned by you. Error: {e}")
    except boto3.exceptions.Boto3Error as e:
        print(f"AWS service error: {e}")
    except ValueError as e:
        print(f"Configuration error: {e}")
    except Exception as e:
        print(f"Unexpected error creating S3 bucket: {e}")

def save_bucket_name(bucket_name):
    """Save the generated bucket name to a JSON file."""
    try:
        with open("resources.json", "w") as file:
            json.dump({"bucket_name": bucket_name}, file)
        print(f"Bucket name '{bucket_name}' saved to resources.json.")
    except Exception as e:
        print(f"Error saving bucket name to resources.json: {e}")
